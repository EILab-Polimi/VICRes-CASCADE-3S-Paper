    ...
    for i in range(len(years_ts)):
        ...
        for n in range(len(ReachData)):
            ...
            for d in range(len(diff)):
                # Line 1: Select the days in the year with the highest yield
                index = np.argsort(-sed_yield_day_tot_n[:, d])  # Sorting in descending order

                # Line 2: Add or subtract the difference in these days, spread so that the added or sub. value is 1m3 per day
                selected_days = index[:int(abs(diff[d]))]  # Convert to integer before using it as an index
