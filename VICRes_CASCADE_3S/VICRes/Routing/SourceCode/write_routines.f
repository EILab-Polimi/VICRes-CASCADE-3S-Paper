...
c     RCS ID STRING
        ...
        INTEGER       RULE, IRRIGATION
        INTEGER       NO_STAS, NAM
        REAL          VOL(NUMRES,DAYS),FLOWIN(NUMRES,DAYS),FLOWOUT(NUMRES,DAYS), VRESER(NUMRES,DAYS),QFLUSHED(NUMRES)
        REAL          HHO(NUMRES,DAYS),HYDROPOWER(NUMRES,DAYS),HTK(NUMRES,DAYS)
        REAL          SEEPAGE, INFILTRATION
        REAL          RES_EVAPORATION(NUMRES,DAYS)
        REAL          HRESERMAX, HRESERMIN
...
c     Subroutine main body
     ...
     &              //'Qspilways_cms Qturbine_cms Energy_MW Qflushed_cms'
                    ...
                    ELSE
                        ...
                        WRITE(40,*) VOL(I,K), VRESER(I,K), HHO(I,K), FLOWIN(I,K), QSPILL, QTUR, HYDROPOWER(I,K), QFLUSHED(I)
                    END DO
                ELSE
                    WRITE(40,*) 'Volume_1000cm FullSupplyVolume WaterLevel_m Qinflow_cms '
     &              //'Qoutflow_cms Energy_MW Qflushed_cms'
                    DO K = 1, DAYS
                        ...
                        WRITE(40,*) VOL(I,2), VRESER(I,K), HHO(I,2), FLOWIN(I,K),
     &                  FLOWOUT(I,K), HYDROPOWER(I,K), QFLUSHED(I)
                    END DO
                END IF
                CLOSE(40)
        END DO
