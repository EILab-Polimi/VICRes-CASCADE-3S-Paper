...
c       Declare variables
        ...
        INTEGER     STARTFLAG(NUMRES,NREACH),DRAWDOWNFLAG(NUMRES,NREACH),FLUSHFLAG(NUMRES,NREACH), FLUSHFAILURE(NUMRES,NREACH),REFILLFLAG(NUMRES,NREACH)
        INTEGER     NYR, FAILUREFLUSH
        INTEGER     RESER(NCOL,NROW)
        INTEGER     IDAY(DAYS), IMONTH(DAYS), IYEAR(DAYS)
        INTEGER     MO(12*NYR),YR(12*NYR)
        INTEGER     DIREC(NCOL,NROW,2)
        ...
        REAL        NEWFLOW(NUMRES, NREACH), QFLUSHED(NUMRES,NREACH),VOL(NUMRES,DAYS,NREACH), FLOWOUT(NUMRES,DAYS,NREACH)
        REAL        HHO(NUMRES,DAYS), ENERGYPRO(NUMRES,DAYS), HTK(NUMRES,DAYS), H0(NUMRES,NREACH, 2), HSED(NUMRES,NREACH)
        ...
        REAL        QRESER(NUMRES,NREACH),VRESER(NUMRES,NREACH,DAYS),SEDVOL(NUMRES,NREACH,2),FLUSHEDVOLUMES(NUMRES,NREACH),REALHEAD(NUMRES,NREACH),VRESERTHAT(NUMRES,NREACH),VDEAD(NUMRES,NREACH)
...
c New variables for connecting to Socket interface and exchange data
...
c       Look for reservoirs sequences
        ...
        DO W=1,NREACH
             DO J = 1,NO_STAS(W)-1
c                  PRINT*,W, J, NAMERS5(W,RESORDER(W,J))
     ...
     &              RESFLOWS(:,:,W), FLOWOUT(:,:,W), HHO, RESFLOWS(:,:,W),N,RES_EVAPORATION, NO_STAS(W),NUMRES)
            END DO
        WRITE(WTEMP, 10) W
 10     FORMAT(I4)
        ...
        DO I = 1, NDAY
            ...
            DO W=1, NREACH
                ...
                IF (CURRENTYEAR>=OPEYEAR(J,W)) THEN
                    ...
                    ELSE
                        IF (VRESER(J,W,I-1) .LT. 0.002) THEN
                            VRESER(J,W,I) = VRESERTHAT(J,W)+VDEAD(J,W)
...
c               CHECK WHETHER DRAWDOWN CAN START
                IF (SEDRULE(J,W) .EQ. 1) THEN
                    ...
                    IF (STARTFLAG(J,W) .EQ. 1) THEN
                        CURRENTWL(W) = (VOL(J,I,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)
c                       WE ARE DOING SEDIMENT MANAGEMENT: SUBDIVIDED INTO 3 STEPS (DRAWDOWN, FLUSHING AND REFILL)
                        IF (DRAWDOWNFLAG(J,W) .EQ. 1) THEN !WE ARE EMPTYING THE RESERVOIR
                            ...
                            IF (CURRENTWL(W) .LT. (MAXWL(J,W)-H0(J,W,1))) THEN !DRAWDOWN IS COMPLETED AND FLUSHING CAN START
                                ...
                                FLOWOUT(J,I,W) = 0
                                VOL(J,I+1,W) = VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6-QFLUSHED(J,W)*24*3.6
...
c                               No Negative Volume 
                                IF (VOL(J,I+1,W)<0) THEN
                                    QFLUSHED(J,W) = (VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6)/24/3.6
                                    VOL(J,I+1,W) = 0
                                END IF
C                               Check whether drawdown is higher than maximum drawdown rate (2 m)
                                IF ((CURRENTWL(W)-(VOL(J,I+1,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)) .GT. MAXRATE(J,W)) THEN 
                                    VOL(J,I+1,W) = (CURRENTWL(W)-MAXRATE(J,W))*VRESERTOT(J,W)/HPROP(J,W)-SEDVOL(J,W,2)
                                    QFLUSHED(J,W) = (VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6 - VOL(J,I+1,W)-FLOWOUT(J,I,W)*24*3.6)/24/3.6 
                                END IF                                 
                            ELSE
                                IF(CURRENTWL(W) .GT. (HRESERMIN(J,W)-H0(J,W,1))) THEN !We can also use turbines to empty the reservoir
                                    QFLUSHED(J,W) = QRESERFLUSH(J,W)
                                    FLOWOUT(J,I,W) = QRESER(J,W)
                                    VOL(J,I+1,W) = VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6-QRESER(J,W)*24*3.6-QFLUSHED(J,W)*24*3.6
c                                   Just to be sure, we check the necessity to spill water 
                                    IF (VOL(J,I+1,W)>VRESER(J,W,I)) THEN
                                        FLOWOUT(J,I,W) = FLOWOUT(J,I,W)+(VOL(J,I+1,W)-VRESER(J,W,I))/24/3.6
                                        VOL(J,I+1,W) = VRESER(J,W,I)
...
C                                   Check whether drawdown is higher than maximum drawdown rate (2 m)
                        ...
                        ELSE IF (FLUSHFLAG(J,W) .GT. 0) THEN
                            ...
                            END IF
                            IF (FLUSHFLAG(J,W) .EQ. DURATION(J,W)+1 .OR. FLUSHFAILURE(J,W) .EQ. FAILUREFLUSH) THEN
                                FLUSHFLAG(J,W)=0
                                FLUSHFAILURE(J,W)=0
                                REFILLFLAG(J,W)=1
                                QFLUSHED(J,W) = 0
                                FLOWOUT(J,I,W) = 0
                                VOL(J,I+1,W) = VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6
                            ELSE
                                QFLUSHED(J,W) = QRESERFLUSH(J,W)
                                VOL(J,I+1,W) = VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6-QFLUSHED(J,W)*24*3.6
c                               No Negative Volume 
...
C                               Check whether drawdown is higher than maximum drawdown rate (2 m)
                                ...
                                IF ((CURRENTWL(W) .LT. MAXWL(J,W)-H0(J,W,1)) .AND. (QFLUSHED(J,W) .GT. MINDISCH(J,W))) THEN !ENOUGH ENERGY TO MOBILIZE DEPOSITED SEDIMENT AND PERFORM FLUSHING
                                    ...
                                    FLUSHFAILURE(J,W) = FLUSHFAILURE(J,W)+1 !COUNTING THE NUMBER OF FLUSHING DAYS
                                    IF (W .EQ. 463) THEN
                                        !WRITE(*,*)'EFFECTIVE FLUSHING',FLUSHFLAG(J,W)
                                    END IF
                                ELSE 
                                    FLUSHEDVOLUMES(J,W) = 0
                                    FLUSHFAILURE(J,W) = FLUSHFAILURE(J,W)+1
                                    IF (W .EQ. 463) THEN
                ...
                ELSE IF (SEDRULE(J,W) .EQ. 2) THEN
                     ...
                     END IF
                     IF (STARTFLAG(J,W) .EQ. 1) THEN
c                        WE ARE DOING SEDIMENT MANAGEMENT: SUBDIVIDED INTO 3 STEPS (DRAWDOWN, SLUICING AND REFILL)
                         IF (DRAWDOWNFLAG(J,W) .EQ. 1) THEN !WE ARE EMPTYING THE RESERVOIR
                             ...
                             IF (CURRENTWL(W) .LT. (TARGETWL(J,W)+0.5)) THEN !PARTIAL DRAWDOWN IS COMPLETED AND SLUICING CAN START
                                 FLUSHFLAG(J,W)=1
                                 DRAWDOWNFLAG(J,W)=0
                                 IF ((VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6+ SEDVOL(J,W,2))>(TARGETWL(J,W)*VRESERTOT(J,W)/HPROP(J,W))) THEN           !Case 2
                                     VOL(J,I+1,W)=TARGETWL(J,W)*VRESERTOT(J,W)/HPROP(J,W)-SEDVOL(J,W,2)
                                     FLOWOUT(J,I,W)=RESFLOWS(J,I,W)-(VOL(J,I+1,W)-VOL(J,I,W))/24/3.6
                             ...
                             ELSE
                                 IF (TARGETWL(J,W)<HSED(J,W)) THEN  ! We can not reach the designWL due to sediment deposition
                                     IF((VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6 -QRESER(J,W)*24*3.6+SEDVOL(J,W,2))>SEDVOL(J,W,2)) THEN
                                         VOL(J,I+1,W) = VOL(J,I,W) + RESFLOWS(J,I,W)*24*3.6-QRESER(J,W)*24*3.6
                                         FLOWOUT(J,I,W) = QRESER(J,W)
                                     ELSE
                                         VOL(J,I+1,W)=0
                                         FLOWOUT(J,I,W)=(VOL(J,I,W)-VOL(J,I+1,W))/24/3.6 + RESFLOWS(J,I,W)
                                     END IF
                                 ELSE                            ! Zone 3
                                     IF ((VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6 -QRESER(J,W)*24*3.6+SEDVOL(J,W,2))                       ! Case 2
...
c                            No Negative Volume 
                             IF (VOL(J,I+1,W)<0) THEN
                                FLOWOUT(J,I,W) = (VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6)/24/3.6
                                VOL(J,I+1,W) = 0
                             END IF   
                         ELSE IF (FLUSHFLAG(J,W) .GT. 0) THEN
                             IF (W .EQ. 463) THEN
     ...
     &                                   >(TARGETWL(J,W)*VRESERTOT(J,W)/HPROP(J,W))) THEN
                         ...
                         ELSE IF (REFILLFLAG(J,W) .EQ. 1) THEN
                             IF (W .EQ. 463) THEN
                                 !WRITE(*,*)'REFILL AND STOP SLUICING'
                             END IF
                             STARTFLAG(J,W)=0
                             REFILLFLAG(J,W)=0
                             FLOWOUT(J,I,W) = 0
...
c     &                     (OP1(J,2,W)-CRTDATE)
     ...
     &                      (CRTDATE-OP1(J,1,W))+(HMAX(J,W)-HMIN(J,W))
                    ...
                    END IF
                    HTK(J,I) = DESIGNWL(W) + H0(J,W,1)													! water head
                    CURRENTWL(W) = (VOL(J,I,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)
     ...
     &                      >(DESIGNWL(W)*VRESERTOT(J,W)/HPROP(J,W))) THEN
                                ...
                                FLOWOUT(J,I,W) = QRESER(J,W)
                            ELSE																	! Case 1
                                VOL(J,I+1,W)=DESIGNWL(W)*VRESERTOT(J,W)/HPROP(J,W)-SEDVOL(J,W,2)
                                FLOWOUT(J,I,W)=(VOL(J,I,W)-VOL(J,I+1,W))/24/3.6 + RESFLOWS(J,I,W)
                            END IF
                        END IF
                        GOTO 123
                    ELSE																		! Zone 2
                        IF ((VOL(J,I,W)+RESFLOWS(J,I,W)*24*3.6+ SEDVOL(J,W,2))>(DESIGNWL(W)*VRESERTOT(J,W)/HPROP(J,W))) THEN           !Case 2
                            VOL(J,I+1,W)=DESIGNWL(W)*VRESERTOT(J,W)/HPROP(J,W)-SEDVOL(J,W,2)
                            FLOWOUT(J,I,W)=RESFLOWS(J,I,W)-(VOL(J,I+1,W)-VOL(J,I,W))/24/3.6
                            IF (FLOWOUT(J,I,W)>QRESER(J,W)) THEN
                                VOL(J,I+1,W)=VOL(J,I+1,W)+(FLOWOUT(J,I,W)-QRESER(J,W))*24*3.6
        ...
        ! Note x1 and x4 in radian (0 to pi/2), not degree
                    ...
                    ELSE IF (VOL(J,I,W) .LE. X2(J,W)) THEN ! hedging
                        IF ((VOL(J,I,W)-VDEAD(J,W)+RESFLOWS(J,I,W)*24*3.6) .LE. (Demand(J,W)+(VOL(J,I,W)-X2(J,W))*tan(X1(J,W))*24*3.6)) THEN		! discharge more than the water amount in reservoir (case 2)
                            FLOWOUT(J,I,W) = (VOL(J,I,W)-VDEAD(J,W))/24/3.6+RESFLOWS(J,I,W)											! discharge all of water in the reservoir
                        ELSE	! (case 3)
                    ...
                    IF (FLOWOUT(J,I,W)>QRESER(J,W)) THEN			! double check just in case users chose unrealistic x1 and x4
                        FLOWOUT(J,I,W) = QRESER(J,W)
                    END IF
                    VOL(J,I+1,W) = VOL(J,I,W) + (RESFLOWS(J,I,W)-FLOWOUT(J,I,W))*24*3.6
                    GOTO 123
                ELSE IF (RULE(J,W) .EQ. 4) THEN! pre-defined time series (Option 4)
                    ...
                    END IF
                    IF (OP2(J,I+STARTDAY(J,W))*24*3.6 .GT. ((VOL(J,I,W)-VDEAD(J,W)))+RESFLOWS(J,I,W)*24*3.6) THEN
                        FLOWOUT(J,I,W) = (VOL(J,I,W)-VDEAD(J,W))/24/3.6 + RESFLOWS(J,I,W)
                    ELSE
...
c               Check the neccesity to spill water
                IF (VOL(J,I+1,W)>VRESER(J,W,I)) THEN
                    ...
                    VOL(J,I+1,W) = VRESER(J,W,I)
                END IF
                IF (VOL(J,I,W) .EQ. 0) THEN
                    HHO(J,I)=(VOL(J,I,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)-HSED(J,W) + H0(J,W,2)
                    ENERGYPRO(J,I) = ENERGYPRO(J,I) *
     &          (( H0(J,W,1)+H0(J,W,1))/2-(HRESERMAX(J,W)-REALHEAD(J,W)))/1000
                ELSE IF (VOL(J,I+1,W) .EQ. 0) THEN
                    HHO(J,I)=(VOL(J,I,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)-HSED(J,W) + H0(J,W,2)
                    ENERGYPRO(J,I) = ENERGYPRO(J,I) *
     &          (( H0(J,W,1)+H0(J,W,1))/2-(HRESERMAX(J,W)-REALHEAD(J,W)))/1000
                ELSE
                    HHO(J,I)=(VOL(J,I,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)-HSED(J,W) + H0(J,W,2)
                    HHO(J,I+1)=(VOL(J,I+1,W)+SEDVOL(J,W,2))*HPROP(J,W)/VRESERTOT(J,W)-HSED(J,W) + H0(J,W,2)
     ...
     &              (( HHO(J,I)+HHO(J,I+1))/2-(HRESERMAX(J,W)-REALHEAD(J,W)))/1000         ! this part is for hydropower production estimation, ignore if work with irrigation reservoirs
                ...
                IF (RES_DIRECT(J,2,W) .EQ. 0) THEN
                    RESFLOWS(NO_STAS(W),I+1+INT(TRVLTIME(J)),W) = RESFLOWS(NO_STAS(W),I+1+INT(TRVLTIME(J)),W) + FLOWOUT(J,I,W) + QFLUSHED(J,W)
                ELSE
                    RESFLOWS(RESORD(W),I+1+INT(TRVLTIME(J)),W) = RESFLOWS(RESORD(W),I+1+INT(TRVLTIME(J)),W) + FLOWOUT(J,I,W) + QFLUSHED(J,W)
                END IF
      !   WRITE(*,*) '-------------------------------------------------'
      !   WRITE(*,*) 'Hdesign ',HTK(J,I,I)
      !   WRITE(*,*) 'CRTDATE',CRTDATE,'Reservoir No.',J,' FLOWIN ',RESFLOWS(J,I,W),
     &!  'FLOWOUT',FLOWOUT(J,I),' VOL ',VOL(J,I),' ELEC', ENERGYPRO(J,I)
                ! In VOLCASCADE we placed information about each reservoirs that are going to be read from CASCADE:
                IF (W .EQ. 463) THEN
                    IF (SEDRULE(J,W) .EQ. 1) THEN
                        IF (DRAWDOWNFLAG(J,W) .EQ. 1) THEN
                            VOLCASCADE(J) = -1 !If cascade receives -1, it knows that this reservoir is performing drawdown
                        ELSE IF (FLUSHFLAG(J,W) .GT. 0) THEN
                            VOLCASCADE(J) = FLUSHEDVOLUMES(J,W) !If receives flushflag is greater than zero, it knows it will receive the flushed volumes
                        ELSE IF (REFILLFLAG(J,W) .EQ. 1) THEN
                            VOLCASCADE(J) = -2 ! When it receives -2, it knows the flushing event is finished
                        ELSE 
                            VOLCASCADE(J) = VRESER(J,W,I) !Otherwise, it is receiving the normal volume for TE calculation
                        END IF
                    ELSE IF (SEDRULE(J,W) .EQ. 2) THEN ! Same holds for sluicing, note that we are always using flushflag even when we are performing sluicing
                        IF (DRAWDOWNFLAG(J,W) .EQ. 1) THEN
                            VOLCASCADE(J) = VRESER(J,W,I)
                        ELSE IF (FLUSHFLAG(J,W) .EQ. 1) THEN
                            VOLCASCADE(J) = -RESERLENGTH(J,W) ! here, we need to provide the length of the reservoir, for the calculation of the TE with churcill
                        ELSE IF (FLUSHFLAG(J,W) .GT. 1) THEN
                            VOLCASCADE(J) = VOL(J,I,W)    
                        ELSE IF (REFILLFLAG(J,W) .EQ. 1) THEN
                            VOLCASCADE(J) = -2
                        ELSE 
                            VOLCASCADE(J) = VRESER(J,W,I) 
            ...
            ENDDO
            ! Now we send the discharges and volumes/sediment management information to CASCADE (step 1), and then we receive its feedback (step 2)
            do g=1,2  
...
c                       write(*,*) "@ message to server: DATAREADY"
                            ...
                            do x=464,NREACH+NUMRES
                                msgbuffer(x)=0
                            enddo
                        ! This loop is just to place the volume of each reservoir in the right place, in such a way that cascade will access this information using the ID of the reservoir as index of the array
                           ...
                           ENDDO 
                        ! We can now send the data to CASCADE
...
Cc                        write(*,*) "@ message to server: HAVEDATA"
                     else ! We receive data from CASCADE
                         call writebuffer(socket, "READY       ", msglen)   ! we are idling and eager to compute something
                         if (.not. isinit) then
...
C                                write(*,*) b
                            !end do
                            ! We update the solid and liquid mass balance of each reservoir, and then we can proceed to next timestep
                            DO W=1, NREACH
                                DO J = 1, NO_STAS(W)-1
                                    Index = 0
                                    ...
                                    IF (CURRENTYEAR>=OPEYEAR(J,W)) THEN
                                        ...
                                        IF (SEDVOL(J,W,2) .LT. 0) THEN
                                            SEDVOL(J,W,2) = 0
                                        END IF 
                                        VRESER(J,W,I) = VRESER(J,W,I)-(SEDVOL(J,W,2)-SEDVOL(J,W,1))
                                        IF (W .EQ. 463) THEN
                                            IF (J .EQ. 28) then
                                                !print*, 'volume of deposited sediment', SEDVOL(J,W,2), Index  !IF FLUSHING IS EFFECTIVE AT TIME I, CASCADE WILL RETURN -FLUSHEDVOLUMES
                                            ENDIF
                                        ENDIF
                                        IF (VRESER(J,W,I) .GT. VRESERTOT(J,W)) THEN
                                            VRESER(J,W,I) = VRESERTOT(J,W)
                                        END IF 
                                        IF (VRESER(J,W,I) .LT. 0.002) THEN
                                            VRESER(J,W,I) = 0.003
                                        END IF 
                                    ENDIF
                                ENDDO
                            ...
                            DO W=1, NREACH
                                DO J = 1, NO_STAS(W)-1
                                    IF (VOL(J,I+1,W)>VRESER(J,W,I)) THEN
                                        NEWFLOW(J,W) = (VOL(J,I+1,W)-VRESER(J,W,I))/24/3.6
                                        VOL(J,I+1,W) = VRESER(J,W,I)
                                        ! Propagate water to the downstream reservoir, considering the time lag
                                        RESORD(W) = NO_STAS(W)
                                        DO K = 1, NO_STAS(W)
